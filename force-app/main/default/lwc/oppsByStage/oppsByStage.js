...
export default class OppsByStage extends LightningElement {
    stageOptions;
    opps;
    error="Please select a stage in order to see matching opportunities";
    columns=COLUMNS;
    ...
    changeHandler(event){
        ...
        .then(result =>{
            if(result.length > 0){
                ...
                this.opps=undefined;
                this.error="There are no mtching opportunities under the selected stage. Please choose another!";
            }

        })
        .catch(error => {
            this.opps=undefined;
            thiis.error=error.body.message;
